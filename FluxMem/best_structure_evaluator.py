...
class BestStructureEvaluator:
    ...
    def find_best_structure(self, ep: list[Page], persona1: list[str], persona2: list[str]):
        ...
        for i in range(3):
            ...
            for question in questions:
                ...
                prompts.append(self._build_response_prompt(query, relevant_pages))
            responses = self._qwen.generate_batch(prompts)
            response_texts = [res.text for res in responses]
            r_judge = self._compute_judge_reward(response_texts, questions)
    ...
    def _compute_judge_reward(self, responses: list[str], questions: list[str]) -> float:
        ...
        IMPORTANT: Your output MUST be a SINGLE float value between 0.0 and 1.0. Output NOTHING else.
        """
