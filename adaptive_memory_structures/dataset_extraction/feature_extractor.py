...
def extract_features(pages: list[Page]) -> np.ndarray:
    ...
    qwen_for_fe = get_client()
    response = None
    info = None
    try:
      response = qwen_for_fe.chat(user=prompt_for_feature_extraction, system="You are a text analyzer. Your task is to analyze the conversation as mentioned and return a valid JSON.", max_new_tokens=1024)
      info = json.loads(response)
    except json.JSONDecodeError:
      print("Warning: Max tokens expanded")
      try:
        response = qwen_for_fe.chat(user=prompt_for_feature_extraction, system="You are a text analyzer. Your task is to analyze the conversation as mentioned and return a valid JSON.", max_new_tokens=2048)
        info = json.loads(response)
      except json.JSONDecodeError:
        print("Warning: Info reduced")
        try:
          response = qwen_for_fe.chat(user=prompt_for_feature_extraction, system="You are a text analyzer. Your task is to analyze the conversation as mentioned and return a valid JSON.\nIMPORTANT: Last response exceeded max tokens. Return only the important information.", max_new_tokens=2048)
          info = json.loads(response)
        except json.JSONDecodeError:
