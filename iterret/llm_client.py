...
class TransformersLLMClient(LLMClient):
    ...
    def chat(self, system_prompt: str, user_prompt: str, *, temperature: float = 0.0) -> str:
        ...
        messages = [{"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_prompt}]
        # return_dict=True -> a BatchEncoding with input_ids (+ attention_mask);
        # transformers>=5 returns this shape, so unpack it into generate() rather
        # than passing the BatchEncoding as a positional "inputs" (which made
        # generate do batch_encoding["shape"] -> KeyError 'shape').
