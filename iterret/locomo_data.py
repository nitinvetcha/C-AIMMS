...
def split_bootstrap_eval(
    ...
    ``ceil(N * bootstrap_fraction)`` (at least 1) seed the experience bank
    (R2-Mem's "10% of it to accumulate experience", App. B.1); the rest are
    held out for evaluation. Order is preserved (not shuffled) so re-runs
    are reproducible without needing to thread a seed through everything."""
